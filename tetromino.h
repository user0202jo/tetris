...
public:
  ...
  Tetromino::Tetromino(std::string name, int size, std::string shape):name_(name), size_(size), original_(nullptr){
  ...
  Tetromino Tetromino::rotatedCW() {
    std::string CWtetromino;
    for(int i = 0; i < size(); i++) {
        for(int j = size()-1; j >= 0; j--) {
          if(shape_[i][j] == true){
            CWtetromino.append("O");
          }
          else{
            CWtetromino.append("X");
          }
        }
    }
    Tetromino result = Tetromino(name(), size(), CWtetromino);
